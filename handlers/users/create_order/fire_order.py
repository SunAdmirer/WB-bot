...
from keyboards.inline.callback_datas import fire_order_cd, confirm_fire_order_cd
from keyboards.default.contacts_fire_order_kb import contacts_fire_order_kb
from keyboards.inline.create_order.cashback_fire_order_kb import cashback_fire_order_kb
from keyboards.inline.create_order.confirm_fire_order_kb import confirm_fire_order_kb
from keyboards.inline.create_order.fire_order_kb import fire_order_kb
from keyboards.inline.create_order.goods_cost_fire_order_kb import goods_cost_fire_order_kb
from keyboards.inline.create_order.goods_link_fire_order_kb import goods_link_fire_order_kb
from keyboards.inline.create_order.goods_name_fire_order_kb import goods_name_fire_order_kb
from keyboards.inline.create_order.order_description_fire_order_kb import order_description_fire_order_kb
from loader import dp
from utils.db_api.commands.orders_cmds import add_order, update_order_goal_amount_order_cost_by_id, \
    update_order_goods_name_by_id, \
    update_order_description_by_id, update_order_goods_cost_by_id, update_order_goods_link_by_id, \
    update_order_contacts_by_id, \
    get_order_by_id, update_order_confirmed_by_id, update_order_cashback_by_id
...
async def fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("fire_order")
    ...
    text = f"Услуга: \n" \
           ...
           f"Описание: каждый пользователь выкупает товар, оставляет к нему отзыв и добавляет в избранное.\n\n" \
           f"Комиссия сервиса: {price} р за 1 выкуп\n\n" \
           f"👇🏻 Выберите или отправьте сообщением необходимое кол-во выполнений:"

    markup = await fire_order_kb()

    await call.message.edit_text(text=text, reply_markup=markup)


# Навигация в выборе кол-во выполнений 🔥 Выкуп + отзыв + ❤
@dp.callback_query_handler(NotBanned(), fire_order_cd.filter(), state="fire_order")
async def fire_order_nav(call: types.CallbackQuery, callback_data: dict, user: Users, state: FSMContext,
                         **kwargs):
...
@dp.message_handler(NotBanned(), regexp=compile(r"^\d+$"), state="fire_order")
async def valid_input_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    ...
    await goods_name_fire_order(call=call, user=user, state=state)
...
# Просим ввести название товара
async def goods_name_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("goods_name_fire_order")

    text = f"Услуга: \n" \
           f"🔥 Выкуп + отзыв + избранное\n\n" \
           f"👇🏻 Отправьте название товара (до 40 символов)"

    markup = await goods_name_fire_order_kb()

    await call.message.edit_text(text=text, reply_markup=markup)


# Корректный ввод названия товара с клавиатуры
@dp.message_handler(NotBanned(), regexp=compile(r"^.{0,40}$"), state="goods_name_fire_order")
async def valid_input_goods_name_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    call: types.CallbackQuery = (await state.get_data()).get("call")
    current_order_id = int((await state.get_data()).get("current_order_id"))
    ...
    await order_description_fire_order(call=call, user=user, state=state)


# Некорректный ввод названия товара с клавиатуры
@dp.message_handler(NotBanned(), regexp=compile(r"^.{40,4000}$"), state="goods_name_fire_order")
async def invalid_input_goods_name_favorites_order_(message: types.Message, user: Users, state: FSMContext, **kwargs):
    print('1')
...
async def order_description_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("order_description_fire_order")

    text = f"Услуга: \n" \
           f"🔥 Выкуп + отзыв + избранное\n\n" \
           f"👇🏻 Отправьте подробное описание задания (до 400 символов)\n\n" \
           f"К примеру:\n" \
           f"\"Выкупить товар, оставить отзыв, указав на его качество и доступность цены\""

    markup = await order_description_fire_order_kb()

    await call.message.edit_text(text=text, reply_markup=markup)


# Корректный ввод описание заказа с клавиатуры
@dp.message_handler(NotBanned(), regexp=compile(r"^.{0,400}$"), state="order_description_fire_order")
async def valid_input_order_description_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    call: types.CallbackQuery = (await state.get_data()).get("call")
    current_order_id = int((await state.get_data()).get("current_order_id"))
    ...
    await goods_cost_fire_order(call=call, user=user, state=state)


# Некорректный ввод описание заказа с клавиатуры
@dp.message_handler(NotBanned(), regexp=compile(r"^.{400,4000}$"), state="order_description_fire_order")
...
async def goods_cost_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("goods_cost_fire_order")

    text = f"Услуга: \n" \
           f"🔥 Выкуп + отзыв + избранное\n\n" \
           f"👇🏻 Отправьте цену товара (только число)"

    markup = await goods_cost_fire_order_kb()

    await call.message.edit_text(text=text, reply_markup=markup)


# Корректный ввод стоимости товара с клавиатуры
@dp.message_handler(NotBanned(), regexp=compile(r"^\d*$"), state="goods_cost_fire_order")
async def valid_input_goods_cost_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    call: types.CallbackQuery = (await state.get_data()).get("call")
    current_order_id = int((await state.get_data()).get("current_order_id"))
    ...
    await cashback_fire_order(call=call, user=user, state=state)
...
# Просим ввести кэшбек
async def cashback_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("cashback_fire_order")

    text = f"Услуга: \n" \
           f"🔥 Выкуп + отзыв + избранное\n\n" \
           f"👇🏻 Отправьте кэшбек товара (только число от 50% и до 100%)"

    markup = await cashback_fire_order_kb()
...
async def valid_input_cashback_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    ...
    await goods_link_fire_order(call=call, user=user, state=state)
...
# Просим ввести ссылку на товар
async def goods_link_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("goods_link_fire_order")
    ...
    markup = await goods_link_fire_order_kb()
...
@dp.message_handler(NotBanned(), state="goods_link_fire_order")
async def valid_input_goods_link_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    call: types.CallbackQuery = (await state.get_data()).get("call")
    current_order_id = int((await state.get_data()).get("current_order_id"))
    ...
    await contacts_fire_order(call=call, user=user, state=state)
...
# Просим контакт для связи
async def contacts_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("contacts_fire_order")
    ...
    markup = await contacts_fire_order_kb()
...
@dp.message_handler(NotBanned(), regexp=compile(r"^\+?\d{0,20}$"),
                    state="contacts_fire_order")
async def valid_input_contacts_fire_order(message: types.Message, user: Users, state: FSMContext, **kwargs):
    call: types.CallbackQuery = (await state.get_data()).get("call")
    current_order_id = int((await state.get_data()).get("current_order_id"))
    ...
    await confirm_fire_order(call=call, user=user, state=state)
...
# Просим подтвердить заказ
async def confirm_fire_order(call: types.CallbackQuery, user: Users, state: FSMContext, **kwargs):
    await state.set_state("confirm_fire_order")
    ...
    markup = await confirm_fire_order_kb()

    await call.message.edit_text(text=text, reply_markup=markup)


# Подтверждение заказа 🔥 Выкуп + отзыв + ❤
@dp.callback_query_handler(NotBanned(), confirm_fire_order_cd.filter(), state="confirm_fire_order")
async def confirm_fire_order_nav(call: types.CallbackQuery, callback_data: dict, user: Users, state: FSMContext,
                                 **kwargs):
    current_order_id = int((await state.get_data()).get("current_order_id"))
